...
// Check if level 10 of basic is completed to unlock intermediate
...
if ($row && $row['completed'] == 1) {
    $basic_level10_completed = true;
}

// Check if level 10 of intermediate is completed to unlock advanced
$intermediate_level10_completed = false;
$intermediate_level10_query = "SELECT completed FROM user_progress 
                             WHERE user_id = {$userId} AND class_level = 'intermediate' 
                             AND level_number = 10";
$result = mysqli_query($conn, $intermediate_level10_query);
$row = mysqli_fetch_assoc($result);

if ($row && $row['completed'] == 1) {
    $intermediate_level10_completed = true;
}

// Check if all basic levels are completed to unlock advanced (legacy code - no longer used)
    ...
    <div class="container">
        ...
        <div class="main-content">
            ...
            <div class="class-options">
                ...
                <?php if ($basic_level10_completed): ?>
                ...
                <?php else: ?>
                    <div class="tooltip">
                        <div class="class-option locked">
                            ...
                            <span class="tooltip-text">Complete level 10 of Basic first to unlock Intermediate class</span>
                        </div>
                    </div>
                <?php endif; ?>
                
                <?php if ($intermediate_level10_completed): ?>
                    <a href="class_detail.php?level=advanced" class="class-option">Advanced</a>
                <?php else: ?>
                    <div class="tooltip">
                        <div class="class-option locked">
                            Advanced <i class="fas fa-lock lock-icon"></i>
                            <span class="tooltip-text">Complete level 10 of Intermediate first to unlock Advanced class</span>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
